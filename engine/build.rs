...
const PROMPT_MASK: &[u8] = b"MossPromptMask";

fn main() {
    println!("cargo:rerun-if-changed=src/prompts/moss.md");
    println!("cargo:rerun-if-changed=src/prompts/plan.md");
    println!("cargo:rerun-if-changed=src/prompts/chat.md");
    println!("cargo:rerun-if-changed=src/prompts/compact.md");
    println!("cargo:rerun-if-changed=assets/o200k_base.tiktoken");

    // Hide statically-linked dependency symbols (notably bundled sqlite3_*)
    // from the cdylib so they cannot interpose on libraries already loaded
    // into the host (kitty) process. Only the #[no_mangle] moss_* API stays
    // exported.
    println!("cargo:rustc-link-arg-cdylib=-Wl,--exclude-libs,ALL");

    let prompt = fs::read("src/prompts/moss.md").expect("read src/prompts/moss.md");
    let encoded = prompt
        .into_iter()
        .enumerate()
    ...
    let dest = Path::new(&out_dir).join("default_moss_prompt.rs");
    fs::write(
        dest,
        format!(
