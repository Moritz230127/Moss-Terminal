...
fn completion_entries() -> [(&'static str, &'static str); 15] {
    [
        ...
        (
            "remove-shell-hook",
            t(
                "Remove installed Moss shell hooks",
        ...
        (
            "update-default-kb",
            t("Update the default knowledge base", "更新 Moss 默认知识库"),
        ),
        (
            "memory",
...
pub fn hook() -> String {
    ...
    for (command, description) in completion_entries() {
        output.push_str(&format!(
            "complete -c moss -n __fish_use_subcommand -f -a {command} -d '{description}'\n"
        ));
    }
    output.push('\n');
    output.push_str(
        r#"function __moss_paste
    set -l output (moss --clipboard-paste 2>/dev/null)
    if test $status -eq 0; and test -n "$output"
        if not set -q __moss_image_counter
            set -g __moss_image_counter 0
        end
        set __moss_image_counter (math $__moss_image_counter + 1)
        set output (string replace "Image 1" "Image $__moss_image_counter" -- $output)
...
bind \cv __moss_paste

function __moss_insert_newline
    commandline -f expand-abbr
    commandline -i \n
end

bind ctrl-j __moss_insert_newline
bind \cj __moss_insert_newline
bind -M insert ctrl-j __moss_insert_newline
bind -M insert \cj __moss_insert_newline

function __moss_wrap_fish_prompt
    functions -q __moss_original_fish_prompt; and return
    functions -q fish_prompt; or fish_prompt >/dev/null 2>/dev/null
    functions -q fish_prompt; or return

    functions -c fish_prompt __moss_original_fish_prompt
    function fish_prompt
        if set -q __moss_pending_buffer
            printf '\e[?25l'
        end
        __moss_original_fish_prompt
    end
end

function __moss_replay_buffer
    set -l buffer $argv[1]
...
function __moss_restore_cursor
    printf '\e[?25h'
    set -e __moss_cursor_hidden
end

function __moss_on_prompt --on-event fish_prompt
    set -q __moss_pending_buffer; or return

    set -l buffer $__moss_pending_buffer
    set -e __moss_pending_buffer
    set -e __moss_image_counter

    trap __moss_restore_cursor INT TERM EXIT
    __moss_replay_buffer "$buffer"
    printf '%s' "$buffer" | moss --shell-intercept --shell fish --stdin
    set -l moss_status $status
    trap - INT TERM EXIT
    __moss_restore_cursor
    return $moss_status
end

function __moss_execute_or_continue
...
function __moss_buffer_is_multiline
    test (string split \n -- "$argv[1]" | count) -gt 1
end

function __moss_multiline_has_unknown_command
    set -l buffer $argv[1]
...
function __moss_accept_line
    ...
    if test -z "$trimmed"
        __moss_execute_or_continue
        return
    end

    if not __moss_buffer_is_multiline "$buffer"
        __moss_execute_or_continue
        return
    end

    if not __moss_multiline_has_unknown_command "$buffer"
        __moss_execute_or_continue
        return
    end

    set -e __moss_image_counter
    __moss_wrap_fish_prompt
    set -g __moss_cursor_hidden 1
    history append -- "$buffer"
    set -g __moss_pending_buffer "$buffer"
    commandline -b -- ""
    printf '\e[?25l'
    commandline -f execute
end

bind enter __moss_accept_line
bind \r __moss_accept_line
bind -M insert enter __moss_accept_line
bind -M insert \r __moss_accept_line
...
function fish_command_not_found
    ...
    moss --shell-intercept --shell fish -- $command 2>/dev/null
    return 127
end
"#,
    );
...
pub fn install(paths: &MossPaths) -> Result<()> {
...
pub fn uninstall(paths: &MossPaths) -> Result<bool> {
    ...
    if removed {
        println!(
            "{}: fish",
            t("removed Moss shell hook", "已移除 Moss shell hook")
        );
    }
    Ok(removed)
}
...
mod tests {
    ...
    fn fish_hook_defines_curated_top_level_completions() {
        ...
        let completion_lines = hook
            .lines()
            .filter(|line| line.starts_with("complete -c moss "))
            .collect::<Vec<_>>();
    ...
    fn fish_hook_defines_paste_binding() {
        let hook = hook();
        assert!(hook.contains("__moss_paste"));
        assert!(hook.contains("bind \\cv __moss_paste"));
        assert!(hook.contains("moss --clipboard-paste"));
    }

    #[test]
    fn fish_hook_defines_enter_binding() {
        let hook = hook();
        assert!(hook.contains("__moss_accept_line"));
        assert!(hook.contains("__moss_wrap_fish_prompt"));
        assert!(hook.contains("functions -c fish_prompt __moss_original_fish_prompt"));
        assert!(hook.contains("if set -q __moss_pending_buffer"));
        assert!(hook.contains("__moss_replay_buffer"));
        assert!(hook.contains("__moss_on_prompt --on-event fish_prompt"));
        ...
        assert!(hook.contains("set -g __moss_cursor_hidden 1"));
        assert!(hook.contains("set -e __moss_cursor_hidden"));
        assert!(hook.contains("return $moss_status"));
        assert!(hook.contains("__moss_execute_or_continue"));
        assert!(hook.contains("__moss_buffer_is_multiline"));
        assert!(hook.contains("test (string split \\n -- \"$argv[1]\" | count) -gt 1"));
        assert!(hook.contains("__moss_multiline_has_unknown_command"));
        assert!(hook.contains("type -q -- \"$command\"; or return 0"));
        assert!(hook.contains("set -g __moss_pending_buffer \"$buffer\""));
        ...
        assert!(hook.contains("bind enter __moss_accept_line"));
        assert!(hook.contains("bind \\r __moss_accept_line"));
        assert!(hook.contains("bind ctrl-j __moss_insert_newline"));
        assert!(hook.contains("bind -M insert enter __moss_accept_line"));
        assert!(hook.contains("bind -M insert ctrl-j __moss_insert_newline"));
    }

    #[test]
    fn fish_hook_resets_image_counter_on_command_not_found() {
        let hook = hook();
        assert!(hook.contains("set -e __moss_image_counter"));
    }
