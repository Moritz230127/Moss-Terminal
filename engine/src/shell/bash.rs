...
const BEGIN_MARKER: &str = "# >>> moss bash hook >>>";
const END_MARKER: &str = "# <<< moss bash hook <<<";
...
pub fn hook() -> &'static str {
    ...
    moss --shell-intercept --shell bash -- "$@" 2>/dev/null
    return 127
}
"#
}
...
mod tests {
    ...
    fn bash_hook_does_not_filter_natural_language_symbols() {
        ...
        assert!(!hook.contains("moss_shell_syntax_pattern"));
        assert!(!hook.contains("moss_leading_pattern"));
    }
