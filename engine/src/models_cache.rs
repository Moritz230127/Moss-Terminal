...
fn cache_file(paths: &crate::paths::MossPaths) -> PathBuf {
...
fn fetch_and_cache(path: &PathBuf) -> Result<HashMap<String, HashMap<String, ModelInfo>>> {
    ...
    let text = client
        .get(API_URL)
        .header("User-Agent", "Mozilla/5.0 Moss/0.1")
        .send()?
        .error_for_status()?
        .text()?;
...
pub fn try_load(paths: &crate::paths::MossPaths) {
...
pub fn spawn_background_refresh(paths: crate::paths::MossPaths) {
    let path = cache_file(&paths);
...
fn lookup_reasoning_info(
    ...
    groups.sort_by_key(|right| std::cmp::Reverse(right.1));
...
pub fn refresh_blocking(paths: &crate::paths::MossPaths) -> Result<()> {
    let _refresh = refresh_lock().lock().unwrap();
    if is_loaded() {
        return Ok(());
    }
