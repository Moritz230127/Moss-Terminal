...
pub fn ensure_initialized(paths: &MossPaths, config: &AppConfig) -> Result<()> {
...
pub fn status(paths: &MossPaths) -> Result<DefaultKbStatus> {
    let state = load_state(paths)?;
    Ok(DefaultKbStatus {
...
pub fn notice_if_update_available(paths: &MossPaths) -> Result<Option<String>> {
...
pub fn check_update_if_due(paths: &MossPaths) -> Result<()> {
    let mut state = load_state(paths)?;
...
pub fn update(paths: &MossPaths, config: &AppConfig) -> Result<DefaultKbState> {
    let git = git_command()?;
...
fn default_kb_source_dir() -> PathBuf {
    std::env::var_os("MOSS_DEFAULT_KB_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("/usr/share/moss/default-kb"))
}

fn state_file(paths: &MossPaths) -> PathBuf {
    paths.data_dir.join("default-kb/state.json")
}

fn update_repo_dir(paths: &MossPaths) -> PathBuf {
    paths
        .cache_dir
        .join("default-kb/shorin-archlinux-guide.git")
}

fn legacy_update_repo_dir(paths: &MossPaths) -> PathBuf {
    paths.cache_dir.join("default-kb/shorinwiki.git")
}

fn update_source_dir(paths: &MossPaths) -> PathBuf {
    paths.cache_dir.join("default-kb/update-source")
}

fn cleanup_legacy_update_repo(paths: &MossPaths, repo: &Path) -> Result<()> {
...
fn load_state(paths: &MossPaths) -> Result<DefaultKbState> {
...
fn save_state(paths: &MossPaths, state: &DefaultKbState) -> Result<()> {
    let path = state_file(paths);
...
fn build_update_source(paths: &MossPaths, repo: &Path) -> Result<PathBuf> {
