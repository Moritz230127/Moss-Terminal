...
pub struct DisplayConfig {
    ...
    /// Surface prompt-prefix cache diagnostics (FR-004 cache shape) inline.
    #[serde(default)]
    pub show_cache_diagnostics: bool,
...
struct RawDisplayConfig {
    ...
    #[serde(default)]
    show_cache_diagnostics: Option<bool>,
    #[serde(default)]
...
impl<'de> Deserialize<'de> for DisplayConfig {
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(Self {
            ...
            show_cache_diagnostics: raw.show_cache_diagnostics.unwrap_or(false),
...
pub struct ProviderConfig {
    pub id: String,
    /// Human-readable label. `name` is accepted as an alias so the config
    /// shape documented in specs/需求规格.md FR-005 loads as written; when
    /// absent entirely it falls back to the provider id.
...
pub struct ContextConfig {
    /// Force tier: hard-evict oldest turns at this fraction of the window.
    ...
    /// Soft tier: diagnostics only.
    #[serde(default = "default_soft_ratio")]
    pub soft_ratio: f32,
    /// Snip tier: stale tool results are pruned from rendered history.
    #[serde(default = "default_snip_ratio")]
    pub snip_ratio: f32,
    /// Compact tier: LLM summarisation engages.
...
impl ProviderConfig {
    ...
    pub fn resolved_api_keys(&self, paths: &MossPaths) -> Result<Vec<ResolvedProviderKey>> {
        let mut keys = Vec::new();
        if let Some(api_key) = self.api_key.as_deref() {
            append_resolved_api_keys(&mut keys, api_key, paths)?;
        }
...
fn append_resolved_api_keys(
    ...
    for item in split_api_keys(raw) {
        let value = if let Some(env_name) = item.strip_prefix("$env:") {
            // Process environment first, then <config_dir>/.env (written by
            // moss-setup) so keys work without shell profile changes.
            match std::env::var(env_name) {
                Ok(value) => value,
                Err(_) => {
                    crate::envfile::lookup(&paths.config_dir, env_name).with_context(|| {
                        format!(
                            "environment variable {env_name} is not set (also checked {}/.env)",
...
impl AppConfig {
    pub fn display_language_hint(paths: &MossPaths) -> Option<String> {
    ...
    pub fn load(paths: &MossPaths) -> Result<Self> {
    ...
    pub fn load_or_default(paths: &MossPaths) -> Result<Self> {
    ...
    pub fn init_files(paths: &MossPaths) -> Result<()> {
        paths.create_dirs()?;
        if !paths.config_file.exists() {
    ...
    pub fn save(&self, paths: &MossPaths) -> Result<()> {
        paths.create_dirs()?;
    ...
    pub fn system_prompt(&self, paths: &MossPaths) -> Result<String> {
    ...
    pub fn base_system_prompt(&self, paths: &MossPaths) -> Result<String> {
    ...
    pub fn custom_system_prompt(&self, paths: &MossPaths) -> Result<String> {
    ...
    pub fn prompts_dir_path(&self, paths: &MossPaths) -> PathBuf {
        config_relative_path(paths, &self.prompt.prompts_dir)
    }

    pub fn user_identity_path(&self, paths: &MossPaths) -> PathBuf {
        config_relative_path(paths, &self.prompt.user_identity_file)
    }

    pub fn identities_dir_path(&self, paths: &MossPaths) -> PathBuf {
        config_relative_path(paths, &self.prompt.identities_dir)
    }

    /// Resolve a persona file. The configured prompts_dir wins; when the file
    /// is not there, `<config_dir>/personas/` is also searched so the layout
    /// documented in specs/需求规格.md FR-007 works as written.
    pub fn persona_path(&self, paths: &MossPaths, name: &str) -> PathBuf {
        let primary = self.prompts_dir_path(paths).join(name);
        if primary.exists() {
            return primary;
        }
        let spec_layout = paths.config_dir.join("personas").join(name);
        if spec_layout.exists() {
            return spec_layout;
        }
        primary
    }

    pub fn identity_path(&self, paths: &MossPaths, name: &str) -> PathBuf {
        self.identities_dir_path(paths).join(name)
    }

    pub fn persona_memory_data_dir(&self, paths: &MossPaths, persona: &str) -> PathBuf {
        paths
            .data_dir
            .join("personas")
    ...
    pub fn persona_memory_state_dir(&self, paths: &MossPaths, persona: &str) -> PathBuf {
        paths
            .state_dir
            .join("personas")
            .join(persona_scope_name(persona))
    ...
    pub fn persona_skills_dir(&self, paths: &MossPaths, persona: &str) -> PathBuf {
        paths
            .skills_dir
            .join("personas")
            .join(persona_scope_name(persona))
    ...
    pub fn active_persona_memory_data_dir(&self, paths: &MossPaths) -> PathBuf {
        self.persona_memory_data_dir(paths, self.prompt.active_persona.trim())
    }

    pub fn active_persona_memory_state_dir(&self, paths: &MossPaths) -> PathBuf {
        self.persona_memory_state_dir(paths, self.prompt.active_persona.trim())
    }

    pub fn active_persona_skills_dir(&self, paths: &MossPaths) -> PathBuf {
        self.persona_skills_dir(paths, self.prompt.active_persona.trim())
    }

    pub fn active_persona_prompt(&self, paths: &MossPaths) -> Result<String> {
    ...
    pub fn user_identity_prompt(&self, paths: &MossPaths) -> Result<String> {
    ...
    pub fn system_prompt_path(&self, paths: &MossPaths) -> PathBuf {
        let value = self
            .system_prompt_file
            .as_deref()
            .map(str::trim)
...
fn config_relative_path(paths: &MossPaths, value: &str) -> PathBuf {
...
fn default_deep_research_dir() -> String {
    if let Some(dirs) = directories::UserDirs::new() {
        if let Some(documents) = dirs.document_dir() {
            return documents.join("Moss/deep-thinking").display().to_string();
        }
    }
    "~/Documents/Moss/deep-thinking".to_string()
}
...
fn default_image_generation_output_dir() -> String {
    if let Some(dirs) = directories::UserDirs::new() {
        if let Some(pictures) = dirs.picture_dir() {
            return pictures.join("moss/generated-images").display().to_string();
        }
    }
    "~/Pictures/moss/generated-images".to_string()
}
...
mod tests {
    ...
    fn mixed_context_window_requires_every_active_model() {
        ...
        provider.models = vec![
            "moss-known-window-model".to_string(),
            "moss-unknown-window-model".to_string(),
        ];
        ...
        config.providers[0]
            .model_context_window
            .insert("moss-unknown-window-model".to_string(), 128_000);
    ...
    fn display_language_hint_reads_jsonc_without_loading_full_config() {
        ...
        let paths = MossPaths {
            ...
            fish_hook_file: temp.path().join("moss.fish"),
            bash_hook_file: temp.path().join("moss.bash"),
            zsh_hook_file: temp.path().join("moss.zsh"),
    ...
    fn meme_library_defaults_follow_persona() {
        ...
        assert_eq!(memes.library_for_persona(""), "moss");
        assert_eq!(
