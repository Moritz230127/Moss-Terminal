...
impl StateStore {
    pub fn new(paths: &MossPaths) -> Result<Self> {
    ...
    pub fn init_files(&self) -> Result<()> {
        ...
        if !self.profile_file().exists() {
            std::fs::write(self.profile_file(), "# Moss Profile\n\n")?;
        }
        Ok(())
    }
    ...
    pub fn reset_if_prompt_changed(&self, system_prompt: &str) -> Result<()> {
        ...
        if previous.trim() != fingerprint {
            // The reset wipes every turn unarchived; keep a file-level backup
            // so a prompt/persona switch can never silently destroy history.
            if !previous.trim().is_empty() {
                let db = self.state_dir.join("conversation.db");
                if db.exists() {
                    let tag: String = previous.trim().chars().take(8).collect();
                    let backup = self.state_dir.join(format!("conversation.pre-{tag}.db"));
                    if let Err(err) = std::fs::copy(&db, &backup) {
                        tracing::warn!("conversation backup before prompt reset failed: {err}");
                    }
                }
            }
            self.conv_db.reset()?;
...
mod tests {
    ...
    fn turn_lifecycle() {
        ...
        let store = StateStore::new(&MossPaths {
        ...
        assert!(!temp.path().join("state/moss.log").exists());
