...
pub fn run(paths: &MossPaths) -> Result<()> {
...
impl TerminalSession {
    ...
    fn run(mut self, paths: &MossPaths, mut config: AppConfig) -> Result<()> {
...
fn run_main_menu(
    ...
    loop {
        ...
        let status = default_kb::status(paths)
            ...
            .map(|_| {
                t(
                    "The default knowledge base needs an update; run moss update-default-kb",
                    "默认知识库需要更新，运行 moss update-default-kb",
                )
            })
            .unwrap_or("");
        draw_menu(
            stdout,
            t(" MOSS CONFIG ", " MOSS 配置 "),
            &options,
            selected,
            status,
        )?;
...
fn edit_personas(stdout: &mut io::Stdout, paths: &MossPaths, config: &mut AppConfig) -> Result<()> {
    ...
    loop {
        ...
        options.push(format!("{default_marker}Moss"));
...
fn remove_persona_scope(paths: &MossPaths, config: &AppConfig, name: &str) -> Result<()> {
...
fn list_identities(paths: &MossPaths, config: &AppConfig) -> Result<Vec<String>> {
    list_markdown_files(&config.identities_dir_path(paths))
}

fn read_identity(paths: &MossPaths, config: &AppConfig, name: &str) -> Result<String> {
    let path = config.identity_path(paths, name);
    if path.exists() {
...
fn write_identity(paths: &MossPaths, config: &AppConfig, name: &str, content: &str) -> Result<()> {
    let path = config.identity_path(paths, name);
...
fn list_personas(paths: &MossPaths, config: &AppConfig) -> Result<Vec<String>> {
...
fn read_persona(paths: &MossPaths, config: &AppConfig, name: &str) -> Result<String> {
    let path = config.persona_path(paths, name);
    if path.exists() {
        Ok(std::fs::read_to_string(path)?)
    } else {
...
fn write_persona(paths: &MossPaths, config: &AppConfig, name: &str, content: &str) -> Result<()> {
    let path = config.persona_path(paths, name);
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
...
impl<'a> ProviderBrowser<'a> {
    fn new(paths: &'a MossPaths, config: &'a mut AppConfig) -> Self {
        Self {
            paths,
            config,
            active_col: 0,
...
fn auto_configure_model_tags(paths: &MossPaths, provider: &mut ProviderConfig, model: &str) {
