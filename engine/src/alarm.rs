...
pub fn alarms_file(paths: &MossPaths) -> PathBuf {
    paths.state_dir.join("alarms.json")
}

pub fn alarm_log_file(paths: &MossPaths) -> PathBuf {
...
pub fn load(paths: &MossPaths) -> Result<Vec<AlarmRecord>> {
...
pub fn save(paths: &MossPaths, records: &[AlarmRecord]) -> Result<()> {
...
pub fn upsert(paths: &MossPaths, record: AlarmRecord) -> Result<()> {
...
pub fn update_status(paths: &MossPaths, id: &str, status: AlarmStatus) -> Result<()> {
    let mut records = load(paths)?;
...
pub fn remove(paths: &MossPaths, id: &str) -> Result<Option<AlarmRecord>> {
    let mut records = load(paths)?;
...
pub fn cleanup_dead(paths: &MossPaths) -> Result<Vec<AlarmRecord>> {
