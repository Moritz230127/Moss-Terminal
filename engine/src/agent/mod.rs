...
use crate::tools::{self, vision, ToolPermission, ToolRegistry};
...
pub struct Agent {
    ...
    paths: MossPaths,
    on_overflow: String,
    overflow_ratios: overflow::OverflowRatios,
    /// Prompt-prefix fingerprint of the previous turn (FR-004 cache shape).
    /// Mutex: written from `chat_with_tools` which only holds `&self`.
    last_cache_shape: Mutex<Option<cache_shape::CacheShape>>,
}
...
impl Agent {
    pub fn new(
        ...
        let overflow_ratios = overflow::OverflowRatios {
            soft: config.context.soft_ratio,
            snip: config.context.snip_ratio,
            compact: config.context.compact_ratio,
            force: config.context.trim_at_ratio,
        }
        .sanitized();
        Ok(Self {
            state,
            ...
            on_overflow,
            overflow_ratios,
            last_cache_shape: Mutex::new(None),
        })
    }

    /// Current context-pressure level on the FR-004 ladder.
    pub fn pressure_level(&self) -> Result<overflow::OverflowLevel> {
        let check = overflow::OverflowCheck::new(self.context_window(), self.trim_at_ratio, None)
            .with_ratios(self.overflow_ratios);
        let tokens = usize::try_from(self.effective_context_tokens()?).unwrap_or(usize::MAX);
        Ok(check.level(tokens))
    }
    ...
    pub fn effective_context_tokens(&self) -> Result<u64> {
        let messages = self.chat_messages("", "", None)?;
    ...
    fn trim_visible_context(&self) -> Result<Vec<crate::state::StoredConversationEntry>> {
        ...
        // Automatic (pressure-driven) eviction: archival is mandatory.
        archive_and_delete_visible_turns_impl(
            &self.state,
            &self.memory,
            &turns,
        ...
        F: FnMut(AgentEvent) -> Result<()>,
    {
        ...
        let prune_policy = PrunePolicy::for_level(self.pressure_level().unwrap_or_default());
        let mut messages = self.chat_messages(&turn_id, &input, prune_policy)?;
        ...
        F: FnMut(AgentEvent) -> Result<()>,
    {
        ...
        let level = check.level(context_tokens);
        // Compact tier engages at compact_ratio (default 0.8) — earlier than
        // the force/eviction tier — so summarisation usually runs before any
        // history has to be evicted.
        if !check.is_enabled() || level < overflow::OverflowLevel::Compact {
            return Ok(None);
        }
    ...
    fn current_model_supports_vision(&self) -> bool {
        ...
        matches!(
            provider.supports_vision(&provider.default_model),
            Some(true)
        )
    }
        ...
        F: FnMut(AgentEvent) -> Result<()>,
    {
        ...
        loop {
            ...
            // FR-004 cache shape: fingerprint the cacheable request prefix
            // (leading system messages + tool definitions) and report when a
            // change guarantees a provider prompt-cache miss.
            {
                let system_prefix: Vec<String> = messages
                    .iter()
                    .take_while(|m| m.role == "system")
                    .filter_map(|m| match &m.content {
                        Some(ChatContent::Text(t)) => Some(t.clone()),
                        _ => None,
                    })
                    .collect();
                let shape = cache_shape::CacheShape::capture(&system_prefix, &definitions);
                let mut last = self.last_cache_shape.lock().unwrap();
                let change = shape.compare(last.as_ref());
                if !matches!(
                    change,
                    cache_shape::ShapeChange::Stable | cache_shape::ShapeChange::FirstTurn
                ) {
                    tracing::info!("{}", change.describe());
                    if self.config.display.show_cache_diagnostics {
                        ...
                        if tools.can_auto_load_direct_call(&call.function.name) {
                            ...
                            if self.config.tools.persist_loaded_tools {
                                self.state.add_session_loaded_tools(
                                    std::slice::from_ref(&call.function.name),
    ...
    fn chat_messages(
        ...
        current_turn_id: &str,
        current_input: &str,
        // Snip tier of the FR-004 ladder: prune stale tool reports from the
        // rendered history once pressure demands it (never from the DB).
        // Computed by the caller — recomputing it here would recurse through
        // effective_context_tokens, which itself renders the messages.
        prune: Option<PrunePolicy>,
    ) -> Result<Vec<ChatMessage>> {
        // self.system_prompt already resolves to the active persona when one
        // is configured (see AppConfig::base_system_prompt), so the persona
        // must not be appended a second time here.
        ...
        let history_len = turns.iter().filter(|t| !t.is_summary).count();
        ...
        for turn in &turns {
            ...
            if !turn.tool_reports.is_empty() {
                // The newest turns keep full-fidelity tool output; only the
                // older tail is pruned, and only under pressure.
                let turn_policy = prune.filter(|p| history_len - history_index >= p.protect_last);
                messages.push(ChatMessage::system(private_tool_memory_pruned(
                    &turn.tool_reports,
                    turn_policy,
                )));
            }
...
/// Render-time tool-report pruning (FR-004 "Prune").
///
/// Under context pressure, stale tool outputs are replaced in the *rendered*
/// history with head+tail excerpts. The rows in conversation.db are never
/// rewritten — the spec's append-only guarantee is what makes this safe to
/// apply aggressively: dropping pressure back below the snip tier restores
/// full-fidelity rendering automatically.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct PrunePolicy {
    /// Keep the newest N turns' tool reports verbatim.
    protect_last: usize,
    /// Bytes of head/tail excerpt kept for pruned reports.
    head: usize,
    tail: usize,
}

impl PrunePolicy {
    /// Reports smaller than this are never worth pruning.
    const MIN_PRUNE_BYTES: usize = 1024;

    pub(crate) fn for_level(level: overflow::OverflowLevel) -> Option<Self> {
        match level {
            overflow::OverflowLevel::Healthy | overflow::OverflowLevel::Soft => None,
            overflow::OverflowLevel::Snip => Some(Self {
                protect_last: 2,
                head: 600,
                tail: 200,
            }),
            overflow::OverflowLevel::Compact => Some(Self {
                protect_last: 2,
                head: 300,
                tail: 120,
            }),
            overflow::OverflowLevel::Force => Some(Self {
                protect_last: 1,
                head: 150,
                tail: 80,
            }),
...
/// UTF-8-safe head+tail excerpt with an explicit elision marker.
fn snip_tool_report(report: &str, head: usize, tail: usize) -> String {
    if report.len() <= head + tail + PrunePolicy::MIN_PRUNE_BYTES {
    ...
    let tail_start = report
        .char_indices()
        .map(|(i, _)| i)
        .find(|i| report.len() - *i <= tail)
        .unwrap_or(report.len());
    let omitted = report[head_end..tail_start].chars().count();
    format!(
        "{}\n[...pruned {omitted} chars of stale tool output; the full text remains in the local conversation database...]\n{}",
        &report[..head_end],
        &report[tail_start..]
    )
}

fn private_tool_memory_pruned(reports: &[String], policy: Option<PrunePolicy>) -> String {
    match policy {
        None => private_tool_memory(reports),
        Some(p) => {
            let snipped: Vec<String> = reports
                .iter()
                .map(|r| snip_tool_report(r, p.head, p.tail))
                .collect();
            private_tool_memory(&snipped)
        }
    }
}
...
fn evicted_turn_entries(
    ...
    for turn in turns {
        ...
        evicted.push(EvictedTurn {
            ...
            timestamp: timestamp.clone(),
            role: "assistant".to_string(),
            // Fold the reasoning into the archived content so eviction never
            // silently discards it (the evicted_turns schema has no separate
            // reasoning column).
            content: match turn
                .assistant_reasoning
                .as_deref()
                ...
                .filter(|r| !r.is_empty())
            {
                ...
                None => turn.assistant_content.clone(),
            },
        });
...
// Used by the CLI history-management commands (feature "cli") and tests.
...
fn archive_and_delete_visible_turns_checked(
    ...
    archive_and_delete_visible_turns_impl(state, memory, turns, expected_loaded_tools, false)
}

fn archive_and_delete_visible_turns_impl(
    state: &StateStore,
    memory: &MemoryStore,
    turns: &[crate::state::Turn],
    expected_loaded_tools: Option<&[(String, Option<String>)]>,
    ...
    } else if require_archive {
        // Automatic eviction must never destroy history it cannot archive
        // (FR-004 append-only guarantee). With memory/evicted-context
        // disabled, skip the eviction; the compact tier carries the load.
        tracing::warn!(
            "context eviction skipped: evicted-context archive is disabled              ({} turns kept; enable memory.evicted_context to allow eviction)",
            turn_ids.len()
        );
        return Ok(Vec::new());
...
mod tests {
    ...
    #[test]
    fn explicit_pop_archives_context_including_reasoning() {
        let temp = tempfile::tempdir().unwrap();
        let paths = test_paths(temp.path());
        let config = AppConfig::default();
        let state = StateStore::new(&paths).unwrap();
        ...
        // Reasoning is folded into the archived assistant content so
        // eviction never silently discards it (FR-004 append-only intent).
        assert!(!memory
    ...
    fn test_paths(root: &std::path::Path) -> MossPaths {
        MossPaths {
            ...
            fish_hook_file: root.join("fish/moss.fish"),
...
mod prune_tests {
    ...
    #[test]
    fn small_reports_are_never_snipped() {
        let report = "short output";
        assert_eq!(snip_tool_report(report, 600, 200), report);
    }

    #[test]
    fn large_reports_keep_head_and_tail_with_marker() {
        let report = format!("HEAD{}TAIL", "x".repeat(10_000));
        let snipped = snip_tool_report(&report, 100, 50);
        assert!(snipped.starts_with("HEAD"));
        assert!(snipped.ends_with("TAIL"));
        assert!(snipped.contains("pruned"));
        assert!(snipped.len() < report.len() / 4);
    }

    #[test]
    fn snip_is_utf8_safe_on_cjk() {
        let report = format!("中文开头{}中文结尾", "汉".repeat(8_000));
        let snipped = snip_tool_report(&report, 100, 50);
        assert!(snipped.contains("中文开头"));
        assert!(snipped.contains("中文结尾"));
        // Must be valid UTF-8 by construction; also must not panic.
        assert!(snipped.contains("pruned"));
    }

    #[test]
    fn policy_maps_ladder_levels() {
        use overflow::OverflowLevel as L;
        assert!(PrunePolicy::for_level(L::Healthy).is_none());
        assert!(PrunePolicy::for_level(L::Soft).is_none());
        let snip = PrunePolicy::for_level(L::Snip).unwrap();
        let force = PrunePolicy::for_level(L::Force).unwrap();
        assert!(force.head < snip.head);
        assert!(force.protect_last <= snip.protect_last);
    }

    #[test]
    fn pruned_rendering_marks_only_when_policy_active() {
        let reports = vec![format!("BIG{}END", "y".repeat(9_000))];
        let full = private_tool_memory_pruned(&reports, None);
        assert!(!full.contains("pruned"));
        let policy = PrunePolicy::for_level(overflow::OverflowLevel::Snip);
        let snipped = private_tool_memory_pruned(&reports, policy);
        assert!(snipped.contains("pruned"));
    }
}
