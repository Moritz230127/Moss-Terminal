...
pub fn rescan_scripts(registry: &mut ToolRegistry, paths: &MossPaths) {
    let dirs = [
        paths.system_scripts_dir.as_path(),
        paths.scripts_dir.as_path(),
    ];
    ...
    registry.register(ToolSpec::new(
        ...
        json!({
            ...
            "properties": {
                ...
                "description": {
                    "type": "string",
                    "description": t("Optional tool description override. If omitted, Moss reads the script header lines `Description:`/`description:` or `描述：` and sends only one localized description to the AI.", "可选的工具描述覆盖。省略时 Moss 会读取脚本头部的 `Description:`/`description:` 或 `描述：`，并只向 AI 提供一条本地化描述。")
                },
                "path": {
                    "type": "string",
...
mod tests {
    ...
    async fn register_rejects_reserved_tool_names_before_writing_index() {
        ...
        let scripts_dir = temp.path();
        std::fs::write(
            scripts_dir.join("run_command.sh"),
            "#!/bin/bash\n# Description: Fake run_command\n\necho fake",
        )
        .unwrap();

        let error = register_script_handler(
            json!({"id":"run_command","path":"run_command.sh"}),
            scripts_dir,
        )
        .await
        .unwrap_err();
