    ...
    let (status, stdout, stderr) = match execution {
        Ok((status, stdout, stderr)) => {
            ...
            let status = match status {
                ...
                // When the engine is embedded (kitty), the host process may
                // reap children process-wide, stealing our exit status
                // (wait -> ECHILD). Both output streams reached EOF, so the
                // command did run to completion — report it with an unknown
                // exit code instead of failing the whole tool call.
...
fn command_output(
    ...
    Ok(serde_json::to_string_pretty(&json!({
        // No status means the host process reaped the child (embedded mode);
        // the streams completed, so treat it as success.
        "success": status.map(|s| s.success()).unwrap_or(true),
...
mod tests {
    ...
    fn readonly_command_allows_inspection() {
        ...
        assert!(ensure_readonly_command("pacman -Q moss").is_ok());
    }
