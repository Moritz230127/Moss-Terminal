...
async fn official_package_query(args: Value) -> Result<String> {
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent("moss-archlinux-official-package-query/0.1")
        .build()?;
...
async fn arch_status() -> Result<String> {
    let client = reqwest::Client::builder()
        ...
        .user_agent("moss-arch-status/0.1")
        .build()?;
    ...
    let latest_event = events.first().map(normalize_status_event);
...
async fn archlinux_news(args: Value, state_dir: &std::path::Path) -> Result<String> {
    ...
    let client = reqwest::Client::builder()
        .timeout(std::time::Duration::from_secs(15))
        .user_agent("moss-archlinux-news/0.1")
        .build()?;
        ...
        match token {
            ...
            XmlToken::Text(text) => {
                ...
                match current_tag {
                    ...
                    "description" if article.description.is_empty() => {
