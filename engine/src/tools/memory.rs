...
async fn recall_past_events(args: Value, config: AppConfig, paths: MossPaths) -> Result<String> {
    let query = required_str(&args, "query")?;
    let limit = optional_limit(&args);
    let store = MemoryStore::new(&config, &paths);
...
async fn remember_fact(args: Value, config: AppConfig, paths: MossPaths) -> Result<String> {
...
async fn recall_memories(args: Value, config: AppConfig, paths: MossPaths) -> Result<String> {
    let query = required_str(&args, "query")?;
    let limit = optional_limit(&args);
...
mod tests {
    ...
    fn test_paths() -> MossPaths {
        let root = PathBuf::from("/tmp/moss-memory-tool-test");
        MossPaths {
