...
pub fn clear_aur_review_state(paths: &MossPaths) -> anyhow::Result<()> {
...
pub fn readonly_registry(config: &AppConfig, paths: &MossPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
...
pub fn chat_registry(config: &AppConfig, paths: &MossPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
    web::register_fetch(&mut registry);
    if config.plugins.web.enabled {
        web::register(&mut registry, config.plugins.web.clone());
...
mod tests {
    ...
    fn test_paths(root: &std::path::Path) -> MossPaths {
        MossPaths {
            ...
            fish_hook_file: root.join("config/fish/conf.d/moss.fish"),
