...
/// Best-effort detection of a provider "context length exceeded" rejection.
///
/// There is no standard error shape across OpenAI-compatible providers, so
/// this scans the error chain's rendered text for the phrasings used by the
/// major implementations (OpenAI, DeepSeek, vLLM, llama.cpp, Anthropic).
/// Used as the force-tier trigger of the FR-004 pressure ladder: when the
/// estimate-driven ladder missed, the provider's own rejection still routes
/// into compaction instead of surfacing a raw API error.
...
mod overflow_error_tests {
    #[test]
    fn detects_common_provider_phrasings() {
        for msg in [
            ...
            assert!(super::is_context_overflow_error(&anyhow::anyhow!("{msg}")));
        }
    }
