...
#[derive(Debug, Parser)]
#[command(name = "moss", version, about = "Moss CLI AI Agent")]
pub struct Cli {
    #[arg(long)]
    pub plan: bool,
...
fn localized_command() -> clap::Command {
    ...
    command = command
        .about(t("Moss CLI AI Agent", "Moss 命令行 AI 助手"))
        .override_usage(t(
            "moss [OPTIONS] [MESSAGE]... [COMMAND]",
            "moss [选项] [消息]... [命令]",
        ));
    if is_zh() {
        command = command
            ...
            .help_template("{about}\n\n用法: {usage}\n\n命令:\n{subcommands}\n参数:\n{positionals}\n选项:\n{options}\n{after-help}")
            .after_help("提示：不带参数进入 REPL；直接输入消息会发送一次对话。可在配置界面设置语言，MOSS_LANG 可临时覆盖。")
            .disable_help_subcommand(true);
    } else {
        command = command
            .after_help(
                "Tip: run without arguments to enter the REPL; pass MESSAGE to send one chat turn. Set the language in the configuration UI; MOSS_LANG is a temporary override.",
            )
            .disable_help_subcommand(true);
    }
...
fn localize_top_args(command: clap::Command) -> clap::Command {
    command
        ...
        .mut_arg("debug", |arg| {
            arg.help(t(
                "Write detailed diagnostics to the Moss log directory",
                "将详细诊断信息写入 Moss 日志目录",
            ))
        })
        .mut_arg("stdout", |arg| {
...
fn localize_subcommands(mut command: clap::Command) -> clap::Command {
    let descriptions = [
        ...
        (
            "remove-shell-hook",
            "Safely remove installed Moss shell hooks",
            "安全删除已安装的 Moss shell hook",
        ),
        ...
        (
            "update-default-kb",
            "Update Moss default knowledge base",
            "更新 Moss 默认知识库",
        ),
        (
            "memory",
...
pub struct AlarmWorkerArgs {
    ...
    #[arg(long)]
    pub time: String,
    #[arg(long, default_value = "Moss alarm")]
    pub label: String,
    #[arg(long)]
...
pub async fn run(cli: Cli, paths: MossPaths) -> Result<()> {
...
async fn run_tool(paths: &MossPaths, mode: AgentMode, args: ToolArgs) -> Result<()> {
...
fn run_init(paths: &MossPaths, kind: InitKind) -> Result<()> {
    let interactive = io::stdin().is_terminal() && io::stdout().is_terminal();
    if interactive {
        println!(
            "{}\n",
            match kind {
                InitKind::FirstRun => t("Moss first start", "Moss 首次启动"),
                InitKind::Explicit => t("Moss initialization", "Moss 初始化"),
            }
        );
    }
    print_init_step(
    ...
    if interactive {
        ...
        println!(
            "{} {}",
            t("initialized Moss at", "Moss 已初始化于"),
...
fn prompt_shell_init_menu(paths: &MossPaths) -> Result<()> {
...
fn remove_shell_hooks(paths: &MossPaths) -> Result<()> {
    ...
    if !removed {
        println!(
            "{}",
            t(
                "no installed Moss shell hooks found",
                "未找到已安装的 Moss shell hook"
            )
        );
    }
    Ok(())
...
fn append_alarm_log(paths: &MossPaths, line: &str) -> Result<()> {
...
fn alarm_worker_paths(state_dir: PathBuf, cache_dir: PathBuf) -> MossPaths {
    MossPaths {
...
fn run_pop(paths: &MossPaths, args: PopArgs) -> Result<()> {
    let config = AppConfig::load_or_default(paths)?;
        ...
        None => {
            if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
                bail!(
                    "{}",
                    t(
                        "interactive pop requires a terminal; use `moss pop <count>`",
                        "交互 pop 需要终端；请使用 `moss pop <数量>`",
                    )
                );
            }
...
fn run_models(paths: &MossPaths, args: ModelsArgs) -> Result<()> {
        ...
        if let Some(active) = inline_fuzzy_select(
            ...
            config.active_provider_models = Some(
                choices
                    ...
                    .map(|(choice, _)| ActiveProviderModelConfig {
...
#[allow(clippy::too_many_arguments)] // established signature; a params struct would add churn, not clarity
...
async fn run_config(paths: &MossPaths, args: ConfigArgs) -> Result<()> {
    match args.command {
...
fn run_clipboard_paste(paths: &MossPaths) -> Result<()> {
    match crate::clipboard::read_clipboard() {
        ...
        Ok(crate::clipboard::ClipboardContent::ImagePath(path)) => {
            ...
            // exists() follows symlinks, so a broken link also reports
            // absent and gets recreated below.
...
async fn run_shell_intercept(paths: &MossPaths, shell_name: &str, message: String) -> Result<()> {
    if !matches!(shell_name, "fish" | "bash" | "zsh") {
...
fn expand_shell_pasted_text_placeholders(paths: &MossPaths, message: &str) -> Result<String> {
...
fn extract_image_placeholders(
    ...
    let cache_images_dir = MossPaths::new()
...
fn run_variant(paths: &MossPaths, args: VariantArgs) -> Result<()> {
    let selected = args
        .name
        .as_deref()
        .map(str::trim)
    ...
    if selected.is_none() && (!io::stdin().is_terminal() || !io::stdout().is_terminal()) {
        bail!(
            "{}",
            t(
                "interactive variant selection requires a terminal; use `moss variant <name>`",
                "交互 variant 选择需要终端；请使用 `moss variant <名称>`",
            )
        );
    }
    ...
    match execute_variant(paths, &mut client, selected, "moss variant")? {
...
async fn run_repl(paths: &MossPaths, initial_mode: AgentMode) -> Result<()> {
    AppConfig::init_files(paths)?;
...
#[allow(clippy::type_complexity)] // local one-off tuple; a named type would only be used once
...
fn resolve_repl_command(input: &str) -> &str {
    if input.starts_with('/') {
...
fn run_history(paths: &MossPaths, args: HistoryArgs) -> Result<()> {
    let state = StateStore::new(paths)?;
...
async fn run_kb(paths: &MossPaths, args: KbArgs) -> Result<()> {
    let config = AppConfig::load(paths)?;
...
async fn run_update_default_kb(paths: &MossPaths) -> Result<()> {
    let config = AppConfig::load_or_default(paths)?;
...
fn run_memory(paths: &MossPaths, args: MemoryArgs) -> Result<()> {
    let config = AppConfig::load_or_default(paths)?;
...
fn run_skills(paths: &MossPaths, args: SkillsArgs) -> Result<()> {
    ...
    match args.command {
        ...
        SkillsCommand::Prune => {
            ...
            for name in skill_names(paths)? {
                ...
                if raw.contains("generated_by: moss") && dir.join(".disabled").exists() {
                    std::fs::remove_dir_all(dir)?;
                    removed += 1;
                }
            }
...
fn skill_names(paths: &MossPaths) -> Result<Vec<String>> {
...
fn skill_dir(paths: &MossPaths, name: &str) -> Result<PathBuf> {
    let clean = name.trim();
    if clean.is_empty()
        || clean.contains('/')
        || clean.contains('\\')
...
fn run_reset(paths: &MossPaths, scope: Option<&str>) -> Result<()> {
    let all = match scope {
        None => false,
        Some("all") => true,
...
mod repl_input_tests {
    ...
    fn pop_test_paths(root: &std::path::Path) -> MossPaths {
        MossPaths {
    ...
    fn models_is_the_cli_model_selector() {
        let matches = localized_command()
            .try_get_matches_from(["moss", "models", "1"])
            .unwrap();
        ...
        let old_matches = localized_command()
            .try_get_matches_from(["moss", "providers"])
            .unwrap();
    ...
    fn variant_is_a_cli_subcommand_with_an_optional_name() {
        let cli = parse_args(["moss", "variant"].map(OsString::from).to_vec()).unwrap();
        assert!(matches!(
            cli.command,
            Some(Command::Variant(VariantArgs { name: None }))
        ));

        let cli = parse_args(["moss", "variant", "high"].map(OsString::from).to_vec()).unwrap();
        assert!(matches!(
            cli.command,
    ...
    fn pop_is_a_cli_subcommand_with_an_optional_count() {
        let cli = parse_args(["moss", "pop"].map(OsString::from).to_vec()).unwrap();
        assert!(matches!(
            cli.command,
            Some(Command::Pop(PopArgs { count: None }))
        ));

        let cli = parse_args(["moss", "pop", "3"].map(OsString::from).to_vec()).unwrap();
        assert!(matches!(
            cli.command,
            Some(Command::Pop(PopArgs { count: Some(3) }))
        ));
        assert!(parse_args(["moss", "pop", "0"].map(OsString::from).to_vec()).is_err());
        assert!(parse_args(["moss", "pop", "nope"].map(OsString::from).to_vec()).is_err());
    }
    ...
    fn debug_is_a_global_cli_option() {
        for args in [
            ...
            &["moss", "models", "--debug", "1"][..],
            &["moss", "hello", "--debug"][..],
            &["moss", "ask", "hello", "--debug"][..],
        ] {
            let cli = parse_args(args.iter().map(OsString::from).collect()).unwrap();
            assert!(cli.debug);
        }

        let cli = parse_args(["moss", "hello", "--debug"].map(OsString::from).to_vec()).unwrap();
        assert_eq!(cli.message, ["hello"]);

        let cli = parse_args(["moss", "--", "--debug"].map(OsString::from).to_vec()).unwrap();
        assert!(!cli.debug);
