...
- **plan**：Moss 已有 `AgentMode::Plan`，不需要在 task 里重复。
...
**不实现 `task_id`**：Moss 没有持久化 session 系统，子代理是一次性的。如果未来需要恢复，可以再扩展。

---

## 5. 返回值格式

采用 JSON 格式（与 Moss 其他工具一致，如 `deep_research`、`linux_game_compatibility`）：
...
**不使用 XML 格式的原因**：
1. Moss 所有其他工具返回 JSON（`deep_research`、`deep_diagnose`、`linux_game` 都是 `serde_json::to_string_pretty(&json!({...}))`）
...
```rust
pub fn builtin_registry(config: &AppConfig, paths: &MossPaths) -> ToolRegistry {
    let mut registry = ToolRegistry::new();
...
**推荐方案 A**，因为 `task` 需要访问 `AppConfig`、`MossPaths` 和 `ToolRegistry`，而 `default_tools::register` 签名只有 `&mut ToolRegistry` 和 `bool`。
